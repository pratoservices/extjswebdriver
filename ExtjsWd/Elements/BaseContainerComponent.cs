...
namespace ExtjsWd.Elements
{
    public abstract class BaseContainerComponent
    {
        ...
        public bool WaitUntilElementIsVisibleInView(IWebElement element, string classname)
        {
            return CreateWebDriverWait(TimeoutInSeconds).Until(IsElementVisibleInView);

            bool IsElementVisibleInView(IWebDriver driver) =>
                (bool)((IJavaScriptExecutor)Driver).ExecuteScript($@"
                var element = document.getElementsByClassName('wd-window-{classname}')[0]
